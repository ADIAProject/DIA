    ...
    Select Case KeyCode
        Case vbKeyUp, vbKeyDown, vbKeyLeft, vbKeyRight, vbKeyPageDown, vbKeyPageUp, vbKeyHome, vbKeyEnd
            If RichTextBoxHandle <> 0 Then
                SendMessage RichTextBoxHandle, wMsg, wParam, ByVal lParam
                Handled = True
            End If
...
If Ambient.UserMode = True Then
    Err.Raise Number:=382, Description:="ScrollBars property is read-only at run time"
Else
    Select Case Value
...
Select Case Value
    Case RtfTextModeRichText, RtfTextModePlainText
        If Ambient.UserMode = False Then PropFileName = vbNullString
        PropTextMode = Value
    Case Else
        Err.Raise 380
End Select
...
UserControl.Refresh
If RichTextBoxHandle <> 0 Then RedrawWindow RichTextBoxHandle, 0, 0, RDW_UPDATENOW Or RDW_INVALIDATE Or RDW_ERASE Or RDW_ALLCHILDREN
End Sub
...
If LineNumber < 0 Then Err.Raise 380
If RichTextBoxHandle <> 0 Then
    If SendMessage(RichTextBoxHandle, EM_LINESCROLL, 0, ByVal LineNumber - 1) <> 0 Then
...
If wMsg = WM_SETFOCUS Then SetFocusAPI RichTextBoxHandle
End Function
