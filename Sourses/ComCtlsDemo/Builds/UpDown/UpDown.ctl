...
DispIDBuddyControl = GetDispID(Me, "BuddyControl")
...
Select Case PropOrientation
    ...
    Case UdnOrientationVertical
        ...
        End If
        If DPICorrectionFactor() <> 1 Then
            .Extender.Move .Extender.Left, .Extender.Top, .ScaleX((WndRect.Right - WndRect.Left), vbPixels, vbContainerSize), .Extender.Height
        Else
            .Size .ScaleX((WndRect.Right - WndRect.Left), vbPixels, vbTwips), .ScaleY(.ScaleHeight, vbPixels, vbTwips)
        End If
End Select
If DPICorrectionFactor() <> 1 Then
    .Extender.Move .Extender.Left + .ScaleX(1, vbPixels, vbContainerPosition), .Extender.Top + .ScaleY(1, vbPixels, vbContainerPosition)
...
If UpDownHandle <> 0 Then
    SendMessage UpDownHandle, UDM_SETPOS32, 0, ByVal PropValue
